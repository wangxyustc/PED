...
from utils.util import update_linear_schedule,base,pro
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                obs, rewards, dones, infos, available_actions,_ = self.envs.step(actions_env)
            ...
            if episode % self.eval_interval == 0 and self.use_eval:
                eval_episode_paths, eval_infos = self.eval(total_num_steps)
                if eval_episode_paths > self.eval_episode_paths:
                    self.eval_episode_paths = eval_episode_paths
    ...
    def eval(self, total_num_steps):
        ...
        for eval_step in range(self.episode_length):
            ...
            for agent_id in range(self.num_agents):
                ...
                if not self.all_args.eval_compare:
            ...
            # Obser reward and next obs
            eval_obs, eval_rewards, eval_dones, eval_infos, eval_available_actions, paths = self.eval_envs.step(eval_actions_env)
            eval_episode_rewards.append(eval_rewards)
            eval_episode_paths.append(paths)
        ...
        eval_episode_paths = np.array(eval_episode_paths).sum(0).mean(0)

        eval_env_infos = {}
        eval_train_infos = []
        eval_env_infos["eval average episode paths"] = eval_episode_paths
        for agent_id in range(self.num_agents):
            eval_average_episode_rewards = np.mean(np.sum(eval_episode_rewards[:, :, agent_id], axis=0))
            eval_train_infos.append({"eval_average_episode_rewards": eval_average_episode_rewards})
            print("eval average episode rewards of agent%i: " % agent_id + str(eval_average_episode_rewards))

        print("eval average episode paths: " + str(eval_episode_paths))
        self.log_train(eval_train_infos, total_num_steps)
        self.log_env(eval_env_infos, total_num_steps)

        return eval_episode_paths,eval_infos
