...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            env = DiscreteActionEnv(all_args.eval_data)

            env.seed(all_args.seed + rank * 624)
            return env
...
def make_eval_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            from envs.env_discrete import DiscreteActionEnv
            env = DiscreteActionEnv(all_args.eval_data)
            env.seed(all_args.seed + rank * 1212)
            return env
...
def parse_args(args, parser):
    ...
    # parser.add_argument("--num_agents", type=int, default=37, help="number of players")
    parser.add_argument("--eval_data", type=str, help="path of eval")
...
def main(args):
    ...
    all_args.num_agents = int(os.path.basename(all_args.eval_data).split('_')[1].split('.')[0])
